...
要启用自然语言 FAE，必须配置仓库中的 `docs/remail-fae-system-prompt.txt` 与
`docs/remail-fae-user-prompt.txt`：前者配置为系统提示词，后者作为当前用户消息模板。系统提示词使用
`<remail_fae_system_v1>` 标记版本；部署时应保留该标记，避免混用旧版规则。插件自身仍会
执行工具路由、事实锁和输出门禁，提示词不能扩大权限或绕过这些检查。
...
插件只在内存中保留同一会话最近 10 分钟内、经过脱敏的上一条 ReMail 问题和最终安全答复，
...
进入循环前，ToolSet 只保留 12 个明确允许的 ReMail 工具，并同时校验工具包装的 handler
...
第三阶段的输出门禁在普通非 seal 路径包含两个相互独立的 LLM。Persona Writer 只读取脱敏问题、主 Agent 草稿
和按计划裁剪后的当前权威 evidence，以严格 JSON 生成完整自然语言候选。它没有工具、历史、
附件或原始私密数据。Writer 返回的 `usedEvidence` 是不可信自报，只能辅助定位，不能证明候选
中的事实已被支持。
...
实际使用的工具全部加入白名单，至少包括 `remail_project_prices`、`remail_recharge_config`
以及项目、库存、FAQ、公告、API 文档和诊断工具；模型和 Provider 必须支持 function calling。AstrBot 默认可能显示
工具调用状态必须按上面的隐私要求关闭，否则插件会拒绝进入 Agent 工具循环。
插件还会在每轮 FAE 请求进入主 Agent 前原地收窄 AstrBot `ToolSet`，只保留本插件注册的 12 个
