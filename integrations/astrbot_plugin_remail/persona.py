...
你是 ReMail FAE“红夜”的独立人格编辑器。你只负责把主 Agent 已经形成的答复改写得冷静、干练、自然、有判断，不负责查询。
...
订单是否收到邮件、是否项目错配、是否退款或未领取，只能来自明确提供的诊断 evidence 和 immutable seal。没有诊断 evidence 时，即使 agentDraft 声称“已到件”“已投递”“在收件箱”“类别不对”或任何同义表达，也必须删除该断言并明确当前无法确认；不得把 agentDraft 当作诊断证据。
...
- 没有明确 DiagnosisFact evidence，却声称邮件已送达、已进入收件箱、项目买错、邮件不匹配、已经退款或未领取；这些规则适用于所有同义表达；
...
只有每条事实声明均得到正确来源的语义支持、关系未改变、候选符合 factPlan 的 answer_mode/privacy/entities 且 requiredEvidence 全部实际支持候选答复时，才可 approve。supportedEvidence 只能列出你确实用于蕴含判断的 evidence id，并必须覆盖 requiredEvidence。
...
def build_persona_payload(
    *,
    ...
    return PersonaPayload(
        question=_public_text(sanitize_feedback_text(question))[:MAX_QUESTION_CHARS],
...
def unsupported_sensitive_states(
    ...
    supported = frozenset().union(
        *(extract_atomic_fact(value).states for value in evidence)
    )
...
def has_unsupported_concrete_facts(
    answer: str, sources: Iterable[str], *, allow_novel_identifiers: bool = False
) -> bool:
    ...
    allowed_literals = _counter_max(allowed, "literals")
    candidate_literals = Counter(candidate.literals)
    unsupported_literals = candidate_literals - allowed_literals
    ...
    return bool(concrete_atoms - allowed_atoms)
...
def parse_critic_response(raw: Any, payload: CriticPayload) -> bool:
    ...
    if (
        ...
        return False
    try:
        response = json.loads(raw, object_pairs_hook=_strict_object)
