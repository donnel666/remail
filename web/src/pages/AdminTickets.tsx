...
export default function AdminTickets() {
  ...
  const canOperateTickets =
    hasPermissionKey(currentUser, "iam:user:operate") ||
    hasPermissionKey(currentUser, "trade:order:operate");
  const canRefundOrders = hasPermissionKey(
    currentUser,
    "trade:order:operate"
  );
  ...
  const openDetail = useCallback(
    (ticketNo: string) => {
      ...
      updateLoadedItems((items) =>
        items.map((item) =>
          item.ticketNo === ticketNo && item.platformUnreadCount > 0
            ? { ...item, platformUnreadCount: 0 }
            : item
        )
      );
    },
  ...
  return (
    <div className="px-2 pt-5">
      ...
      <TicketDetailSheet
        platformCanOperate={canOperateTickets}
        platformCanRefund={canRefundOrders}
