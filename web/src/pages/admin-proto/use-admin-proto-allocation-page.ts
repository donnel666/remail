...
import { listAdminProtoAllocations } from "../../lib/admin-proto-api";

import type { AdminProtoAllocation } from "./admin-proto-types";

export function useAdminProtoAllocationPage(
  resourceId: number
) {
  const [pageSize, setPageSize] = useSharedPageSize();
  const [pagination, setPagination] = useState({ page: 1, resourceId });
  const [items, setItems] = useState<AdminProtoAllocation[]>([]);
  ...
  useEffect(() => {
    ...
    void listAdminProtoAllocations(
      resourceId,
      (page - 1) * pageSize,
      pageSize,
      controller.signal
