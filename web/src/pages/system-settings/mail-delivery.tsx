...
const D = { outbound_mail_timeout_minutes: 3, inbound_mail_timeout_minutes: 2, outbound_mail_claim_timeout_minutes: 2, auxiliary_domain_refresh_interval_seconds: 60, max_inbound_header_runes: 500, max_inbound_preview_runes: 1000, max_inbound_body_bytes: 1048576, max_inbound_body_runes: 200000, max_inbound_mime_depth: 12, mail_dispatcher_interval_seconds: 15, alias_dispatcher_interval_seconds: 2, token_refresh_dispatcher_interval_seconds: 2, legacy_alias_retry_delay_seconds: 30 };
...
export default function MailDeliverySection({ options, onBulkSave }: SectionProps) {
  ...
  return <SettingsSection title={<SettingsCardHeader icon={<Send size={16} />} title={t("邮件投递")} description={t("配置入站解析、外发超时和邮件调度间隔")} />}>
    <SettingsFormGrid className="mt-4">
      ...
      {field("外发邮件认领超时（分钟）", "outbound_mail_claim_timeout_minutes")}
      ...
      <SettingsNumberField label={t("邮件正文最大体积（MB）")} value={number(form.max_inbound_body_bytes) / BYTES_PER_MB} onChange={(value) => update("max_inbound_body_bytes", Math.round(value * BYTES_PER_MB))} min={0} />
      {field("邮件正文最大字符数", "max_inbound_body_runes")}
      {field("MIME 嵌套最大深度", "max_inbound_mime_depth")}
      {field("邮件投递调度间隔（秒）", "mail_dispatcher_interval_seconds")}
