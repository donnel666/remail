...
const D: Record<string, unknown> = { bucket_count: 64, candidate_window_size: 4, global_candidate_window: 8, bucket_probe_count: 4, alias_generation_window: 32, candidate_retry_count: 5, dot_alias_capacity_per_resource: 10, inventory_refresh_interval_minutes: 10, inventory_cache_activity_ttl_minutes: 20, inventory_cache_hard_ttl_hours: 24 };
...
export default function AllocationSection({ options, onBulkSave }: SectionProps) {
  ...
  return <SettingsSection title={<SettingsCardHeader icon={<Gauge size={16} />} title={t("分配引擎")} description={t("配置候选选择、别名容量和库存缓存策略")} />}>
    <SettingsFormGrid className="mt-4">
      ...
      {field("单资源别名上限", "dot_alias_capacity_per_resource")}
