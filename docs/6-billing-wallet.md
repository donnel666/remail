...
| 2026-08-26 | V2.1 | Codex | 适配未定制的同机 EPUSDT：外部订单号限制为 32 位，并通过只读挂载的 SQLite/WAL 主动查账，不依赖 provider 回调或不存在的签名查询路由。 |
| 2026-09-01 | V2.2 | Codex | 支付宝和 USDT 共用系统设置 `recharge_timeout_minutes`，默认 10 分钟；创建、回调、查账和入账使用同一窗口。 |
...
回调不等于入账，只把仍在统一充值超时窗口内的 `paying` 充值单原子标记为 `callback`，并立即投递 realtime 高优先级主动查账；如果已有查账租约，则与在途查账合并，不并发重复请求。`recharge_timeout_minutes` 同时约束支付宝和 USDT，默认 10 分钟；创建响应的 `expiresAt`、待处理订单计数、回调接收、定时扫描、主动查账、最终入账和队列去重均读取这个设置。创建支付订单的响应是不可信的，创建流程只提取并校验支付链接，不依据其中的状态、金额或交易号判断收款。回调请求同样是不可信的，只读取订单号作为查账唤醒信号，回调携带的金额、状态、交易号和签名均不参与资金判断。收到回调的订单立即查账；未收到回调时，EPUSDT 订单创建满 10 秒、EPay 或历史订单创建满 60 秒启动兜底查账。首次查询后前 10 次查询之间间隔 5 秒，第 10 次后间隔 30 秒。EPay 只有主动异步查账通过供应商响应签名校验后才允许入账；同机 EPUSDT 则只信任以只读模式打开的 provider SQLite 持久化状态，并同时校验 API Key PID、订单号、金额、币种、Token、网络、链上交易 ID 和订单类型。未支付状态继续等待，明确失败状态结束订单，查账异常重试。窗口外已付款订单进入人工核验，不自动入账。EPUSDT v2 的建单接口不接受逐单超时，部署配置 `order_expiration_time` 必须与 `recharge_timeout_minutes` 保持一致。每次查询先在 MySQL 原子领取带代次的租约，陈旧任务不能提交普通完成或失败；可信支付成功可以覆盖并发产生的失败终态，但仍受订单金额校验和网关流水唯一约束。充值单保存创建时的只读网关配置快照，密钥不通过 API 返回，后续配置轮换不会影响在途订单。V1 使用 MD5 与扩展后的 `POST /api.php` 订单查询，避免商户密钥进入 URL；易支付侧仍保留原有 GET 兼容。V2 使用 RSA-SHA256 与平台公钥验证查单响应签名，并在判断支付成功前完成验证。

EPUSDT 新订单的支付金额为 `充值积分 / epusdt_points_per_usdt`，向上取两位小数，`topup_fee_rate` 和 `topup_fee_cap` 完全不参与；EPay/支付宝仍使用原有手续费配置。启用 EPUSDT 前必须在管理端填写正的 `epusdt_points_per_usdt`，否则渠道保持不可用。新订单请求 `currency=USDT`，要求部署的 GMPay 版本支持同币种 1:1 汇率；remail 不修改 provider，也不会用未知的 CNY/USDT 市场汇率猜算金额。remail 内部充值号保持 `RC` 加 32 位 UUID，发送给原版 EPUSDT 的 `order_id` 去掉 `RC` 前缀以满足其 32 字符上限；浏览器返回参数仍使用完整内部充值号。旧快照没有显式币种字段时继续按 CNY 协议校验，汇率调整不会改变已创建订单的支付金额。
...
| 数据源 | 说明 |
|--------|------|
| `/run/epusdt/epusdt.db` | 生产 Compose 将 EPUSDT 的整个 runtime 命名卷以只读方式挂载到 remail；连接使用 `mode=ro`、`query_only`，读取活跃 WAL，不使用 `immutable`。按 `orders.order_id` JOIN `api_keys.pid`，完整匹配充值单快照后才可进入钱包入账事务。 |

同机 SQLite 路径由部署环境变量 `EPUSDT_SQLITE_PATH` 指定；配置后数据库缺失、繁忙、结构变化、WAL 不可读或字段不匹配都必须按查账失败重试，禁止退回公共 HTTP 状态接口。未配置本地路径的远程部署仍可使用带响应签名的 `POST /payments/gmpay/v1/order/query`。remail 永远不会把未鉴权的 `/pay/check-status/{trade_id}` 当作资金事实。
...
| ADR-BILL-6 | 同机 EPUSDT 通过只读 SQLite/WAL 查账 | 原版 provider 没有签名查单 API，公共状态接口不具备资金信任条件；只读挂载无需定制 EPUSDT，并能把入账建立在其持久化订单事实及严格字段匹配上。接受 remail 对 provider 表结构和只读数据可见性的耦合；EPUSDT 升级必须先验证 schema，未来需要缩小数据暴露面时改为最小权限本机 sidecar。 |
