...
| 2026-07-17 | V1.40 | Codex | 将 Microsoft 旧项目识别从验证 worker 的同步全量拉取拆为独立 MailMatch 异步任务：验证只轻量探测 Inbox/Junk；扫描按确定性规则识别主邮箱/点号/加号/显式别名，复用既有订单与分配框架补齐超级管理员 0 积分已过保历史订单。 |
...
| 2026-07-22 | V1.42 | Codex | 将显式别名平台 owner 收敛为固定的首个超级管理员 `users.id=1`：所有新增、远端回填和历史回填统一写 1，迁移修正存量并以数据库约束拒绝其他 owner。 |
| 2026-07-26 | V1.43 | Codex | 普通用户出售分流改为创建标题固定的非订单供应商申请工单；管理员看到工单后人工授予 supplier 角色，不再使用独立供应商审批流程。 |
...
| 旧项目识别 | Microsoft 健康结果提交前，Core 先幂等投递独立 `validated_microsoft_history_scan` 任务；投递失败则保持 `validating` 交给现有验证重试。健康结果成功提交后资源进入 `identifying`，不进入分配池。任务 worker 使用已提交凭据流式全量读取 Inbox/Junk：地址等于主邮箱为 `main`，同域去点后相同为 `dot`，同域且 `+` 前缀相同为 `plus`，其余实际收件地址为显式 `alias`。MailMatch 提交识别结果；BC-TRADE 对每条具体邮箱关系先复用既有 Allocation，只有系统尚无该关系时才通过 BC-BILLING 历史审计专用通道与 BC-ALLOC alias/guard/allocation Port，为确定性的 `super_admin` 创建一笔 0 积分、`completed`、已过保且 Allocation 已 `released` 的幂等历史订单；审计通道在关系创建事务内直接写入金额及前后余额均为 0 的流水，不读取、锁定或更新超级管理员钱包，也不创建独立 Billing 幂等回执。已有关系的纯 no-op 不依赖 `super_admin`。这些事实与 `identifying -> normal` 在同一数据库事务提交。失败时保持 `identifying`，管理员可按该状态筛选并重新提交历史扫描。 |
...
普通 `user` 点击出售不改变资源，而是创建 BC-AFTERSALE 非订单工单：`ticketType=general`、标题固定为“供应商申请”、首条消息为用户填写的申请内容。管理员看到工单后通过用户管理人工将其角色改为 `supplier`，不经过独立供应商审批状态机，也不自动发布任何资源。用户成为 `supplier` 后，仍需主动对自有私有资源执行发布出售命令。
...
| 实体 | 用途 | 关键规则 |
|------|------|----------|
| `ExplicitAlias` | 真实创建在 Microsoft 账号里的显式别名 | 为所有 `status=normal` 的 Microsoft 资源自动创建，不区分 `forSale=true/false`；按 Asia/Shanghai 自然周最多 2 个、自然年最多 10 个；成功事实统一归首个超级管理员 `users.id=1` 所有。 |
...
`ExplicitAlias.ownerUserId` 是平台库存所有权，固定等于首个激活的超级管理员 `users.id=1`，不继承 Microsoft 主资源的供应商 owner，也不接受调用方指定。确认或回填显式别名时，事务必须验证 `users.id=1 AND role=super_admin`，然后写入 owner；如果该身份不存在或角色不符，整笔落库必须回滚，不能生成 `NULL` owner 或退化为其他用户。若同一 `resourceId + email` 已存在，远端再次确认成功时也要把 owner 收敛到 1；数据库约束必须拒绝任何非 1 的 `owner_user_id`。显式别名的创建资格与 allocation 供给范围相互独立：私有主资源也可以预创建别名，但不会因此变成公开供给，BC-ALLOC 仍按主资源和下单用户的既有规则选择库存。
...
| `POST` | `/v1/admin/projects/reject` | 批量驳回审核中项目；`selection.mode=ids/filter`，必须提供 `reviewReason`，服务端按集合处理并只返回影响数量。 |
