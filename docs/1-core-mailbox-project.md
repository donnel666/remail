...
| 2026-07-02 | V1.7 | Codex | 补充 P1-I2 普通用户删除自有私有 Microsoft 资源；不改变公开出售单向策略。 |
...
批量发布出售是单个发布命令的批量形式，只接受当前调用者拥有的 Microsoft 资源 ID 列表。服务端必须先校验全部 ID 都属于当前 owner 且为 Microsoft 资源，再只对 `forSale=false` 的资源执行 `forSale=true`；已处于 `forSale=true` 的资源视为幂等跳过，不得报“已售出”。

普通用户和供应商都可以删除自有 `forSale=false` 的 Microsoft 私有资源。删除只适用于尚未发布到公开供给池的资源，服务端必须校验 owner、资源类型和 `forSale=false`；`forSale=true` 表示资源已经进入公开出售供给池，不是已售出状态，用户侧不得删除或下架，后续如需下架必须另行设计管理员命令。删除成功写入 OperationLog，资源根和 Microsoft 子表记录在同一事务内删除。
...
| `DELETE` | `/v1/resources/{resourceId}` | 当前登录用户删除自有 Microsoft 私有资源；仅允许 `forSale=false`。 |
...
| `POST` | `/v1/resources/{resourceId}/publish` | 供应商将自有 Microsoft 私有资源单向发布为公开出售供给。 |
| `POST` | `/v1/resources/publish` | 供应商批量将自有 Microsoft 私有资源单向发布为公开出售供给，已公开资源幂等跳过。 |
