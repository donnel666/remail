...
`ExplicitAlias.ownerUserId` 是平台库存所有权，固定等于首个激活的超级管理员 `users.id=1`，不继承 Microsoft 主资源的供应商 owner，也不接受调用方指定。确认或回填显式别名时，事务必须共享锁定并验证 `users.id=1 AND role=super_admin`，然后写入 owner；如果该身份不存在或角色不符，整笔落库必须回滚，不能生成 `NULL` owner 或退化为其他用户。若同一 `resourceId + email` 已存在，远端再次确认成功时也要把 owner 收敛到 1；数据库约束必须拒绝任何非 1 的 `owner_user_id`。显式别名的创建资格与 allocation 供给范围相互独立：私有主资源也可以预创建别名，但不会因此变成公开供给，BC-ALLOC 仍按主资源和下单用户的既有规则选择库存。
