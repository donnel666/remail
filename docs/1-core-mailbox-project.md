...
| `POST /v1/resources/validations` | 批量检测入口，接收 `selection.mode=ids|filter`，只批量创建或复用 `ResourceValidation(queued/running)` 任务并唤醒 dispatcher，返回 `202 Accepted`；客户端不得为批量检测循环调用单资源验证接口。Domain `filter` 可使用既有 `purpose=not_sale/sale/binding` 筛选，保证“全部检测”与用户侧私有/出售筛选一致。 |
