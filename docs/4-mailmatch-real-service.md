...
### 2.1 `OrderResultSnapshot`

> V1.2 补充设计：这是“已交付结果快照”，不是通用邮件-订单匹配关系表。
...
| `subject` | 主题 |
| `body` | 正文 |
| `verificationCode` | 已交付验证码 |

快照只保存已经唯一命中并交付给订单的结果事实，用于提高读取效率和固定接码结果：
...
| INV-M11 | 订单交付结果快照只在唯一命中后写入，不能表达候选关系或冲突关系。 |
| INV-M12 | 接码订单快照只写一次；购买订单快照只能按更新的 `receivedAt` 前进。 |
| INV-M13 | pickup 可优先读取快照；接码已有快照时不得重复触发外部拉取。 |
...
| ADR-MM-6 | 增加订单交付结果快照 | 接码需要一次性交付结果，购买需要展示最新验证码；快照只保存已交付 6 元素，不改变 `Message` 原始邮件事实和实时匹配策略。 |
