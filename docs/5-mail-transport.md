...
- `DeliveryPort.Send` 把完整 SMTP payload 临时写入独立 Redis key，固定 TTL 为 5 分钟，再投递 `mailtransport:outbound_send` Asynq 引用任务；HTTP/IAM 不同步等待真实 SMTP 网络发送，外发通知不写 `outbound_mails` 或其他 MySQL 状态表。
...
- `mailtransport:outbound_send` 的 Asynq payload 只携带不敏感的哈希引用，不携带收件人、正文或验证码。worker 发送成功或最终失败后都主动删除 Redis payload；进程崩溃、kill 等无法执行清理的异常由 5 分钟 TTL 自动回收。
- Asynq 任务级 `MaxRetry` 固定为 0、`Retention` 固定为 0；单个 worker 任务内部对可重试 SMTP/基础设施失败最多重试 3 次（首次发送加 3 次重试），永久 SMTP 拒绝不重试。最终失败只打印禁敏日志，不保存成功/失败记录。
