...
| P1-I5 | 分配路由 | Microsoft 候选、自建候选、主邮箱/显式别名/点别名/加号别名/生成邮箱分配、OrderGuard、一单一资源、释放、按订单/收件人查询、后台库存诊断和 Microsoft/Domain 候选刷新 | 并发分配不重复；同一订单并发幂等只产生一个分配；同资源不会被两个订单抢占；MailMatch 可按收件人索引定位 active 分配；同步分配事务内不做外部网络调用、不 sleep，死锁由带抖动短事务重试兜底；候选刷新 HTTP 入口只创建持久异步任务并返回 `202` |
