...
| 2026-07-05 | V1.1 | Codex | 按用户验收调整 P1-I4 阶段范围：在最小项目商品基础上纳入项目广场、用户项目申请/重新提交、管理员审批补配置、私有项目授权和项目 Logo；这是阶段范围补充，不改变核心项目状态机。 |
| 2026-07-06 | V1.2 | Codex | 补充 P1-I5 阶段范围：分配路由必须一次补齐 Microsoft/Domain allocation 表、OrderGuard、释放、收件人查询、库存诊断和 Microsoft 候选刷新；此为缺失实施范围细化，不改变 BC-ALLOC 边界。 |
...
| 2026-08-07 | V1.15 | Codex | 收敛 P1-I5：删除不参与分配正确性的 Microsoft/Domain 候选镜像及刷新任务，统一直接查询并锁定权威资源表。 |
...
| 阶段 | 定位 | 核心目标 | 完成标准 |
|------|------|----------|----------|
| 一期：扎实平台基础 | 围绕资源这个核心中的核心，打通可运行、可验证、可继续扩展的基础闭环 | 用户上传 Microsoft 私有资源，supplier 发布出售或上传自建邮箱资源，用户能网页下单和 SDK 下单，订单能分配资源并读取服务结果 | 不靠 SQL，能跑通“资源上传/发布 -> 资源验证 -> 项目商品 -> 用户下单 -> 分配 -> 读取邮件/验证码” |
...
| P1-I4 | 项目广场和项目商品 | 管理员创建可售项目、商品、价格、资源类型、服务窗口、基础邮件规则；普通用户可申请项目、重新提交驳回申请；管理员审批时补齐配置；私有项目通过授权可见；项目支持 Logo | 项目商品能关联 Microsoft/自建资源类型；申请/审批/私有授权作为本阶段能力；状态机仍为 `reviewing/listed/delisted` |
| P1-I5 | 分配路由 | 从权威资源表查询 Microsoft/Domain/Gmail/iCloud 候选，完成各类邮箱形态分配、OrderGuard、一单一资源、释放、按订单/收件人查询和后台库存诊断 | 并发分配不重复；同一订单并发幂等只产生一个分配；同资源不会被两个订单抢占；MailMatch 可按收件人索引定位 active 分配；同步分配事务内不做外部网络调用、不 sleep，死锁由带抖动短事务重试兜底 |
| P1-I6 | 钱包基础 | 用户测试余额、扣款、退款、不可变流水、幂等表 | 余额不负；重复请求不重复扣款 |
| P1-I7 | 订单闭环 | 网页下单、API Key 下单、订单状态、扣款、分配、签发服务凭证 | 网页和 SDK 都能创建订单并拿到分配结果 |
| P1-I8 | 读取结果 | Microsoft Graph 拉取、自建邮箱拉取/接收、MailMatch 保存邮件、匹配验证码、pickup 取件接口 | 持有 `email + token` 的调用者能读取订单过滤后的邮件或验证码；读取接口内部可触发异步拉取 |
...
| 项目商品 | 管理端可直接创建最终项目商品；普通用户可提交项目申请，管理员审批时补齐商品/规则/访问配置；私有项目通过授权进入可见范围。 |
| 下单 | 统一 `POST /v1/orders`，Session/API Key 共用。 |
| 订单读取 | 统一订单详情由 Session/API Key 读取；邮件列表、验证码读取只走 `pickup(email + token)`，控制台和分享链接共用。 |
...
6. user 上传私有 Microsoft 资源并验证默认私有、长短效字段
7. supplier 单个/批量发布自有 Microsoft 资源出售
8. supplier 上传自建邮箱资源
9. admin 创建最小项目商品
10. user 网页下单
11. user 创建 API Key
12. SDK/API Key 下单
13. 两个订单分别完成分配
14. 拉取/接收邮件并读取验证码或邮件结果
15. 断言凭据和代理 URL 不在响应、普通日志和错误 message 中
```
...
| AMR-5 管理命令 | 实现管理员代导入、原子编辑、替换凭据、Validate/Enable/Disable/Publish/Unpublish/Delete/Recover、RT 刷新、alias schedule 加速、资源 Fetch 和 `ids/filter` 批量命令 | Handler 不写业务状态；写操作回到事实所有者；validation 大批量和外部执行走 Redis 临时任务，其他明确要求恢复的流程使用各自 durable fact；失败和 panic 回滚、active Allocation 保护、OperationLog/SystemLog 有证据。 |
...
| AMR-5 | 主体完成、关键正确性门禁已补 | 原子 edit/credentials/state、Redis-only validation fencing、import fencing/items、bulk checkpoint、Token/Alias/Fetch durable tasks 已落盘；rollback/panic/日志失败、active single-flight、分页续跑和锁序测试覆盖主要不变式。Import/Bulk 只对已知 1205/1213 做最多 3 次短事务重试 | Validation 的 cursor/执行/retry 只存在 Redis，数据库只保留资源 `pending/validating/normal/abnormal`；Token/Alias/Fetch 等其他任务仍按各自事实模型恢复。额外压力、全仓 race 和 chaos 仅在真实风险需要时执行，不作为本功能完成条件 |
...
| 多鉴权主体 | 同一接口可支持 Session/API Key，具体由中间件和 handler 标记控制。P1-I8 补充：取件只使用 `pickup(email + token)` 资源钥匙，不作为通用鉴权主体。 |
