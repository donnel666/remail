...
| 验证后历史识别 | 健康结果最终提交前先幂等投递独立 `validated_microsoft_history_scan`，投递失败沿用 validation retry；任务只在资源已为 `normal` 后使用 `background_project_history` 队列全量流式扫描。rotated RT 与规则快照按 credential revision/rule snapshot fencing 校验，识别结果经 BC-TRADE 调用现有 BC-BILLING/BC-ALLOC Port，在同一短事务内提交别名、0 积分历史订单和 released Allocation。 |
