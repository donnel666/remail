...
```text
name@proton.me----password
name@protonmail.com----password
name@proton.me----password----base64(PKL)
name@protonmail.com----password----base64(PKL)
```

- 第一段必须是完整邮箱，仅接受 `proton.me`、`protonmail.com` 两个后缀；只去掉邮箱首尾空白并统一大小写，不自动补全或替换域名。同一用户名的两个后缀视为不同邮箱，按完整地址去重。密码的首尾空格保留；第三段允许首尾空白，内部不能折行或有空白。
...
- CMD 的 `login -file/-stdin` 同样要求完整的两种后缀邮箱并识别第三段，仅做诊断、不落库；带 PKL 只能使用 Python 引擎。正式持久化使用管理员导入流程，普通用户权限不因此开放。
- 管理员列表支持按这两个后缀筛选，沿用现有 `suffix` 参数和后缀统计；不增加商品类型、订单字段或数据库迁移，验证、收件、历史识别和分配机制不变。
