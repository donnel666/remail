...
- `remail_allocation_duration_seconds`、`remail_allocation_results_total`
- `remail_allocation_candidate_attempts_total`、`remail_allocation_resource_lock_skips_total`
- `remail_allocation_candidate_recheck_misses_total`、`remail_allocation_bucket_fallbacks_total`

Allocation 新建成功 P95、新建吞吐、幂等命中吞吐、总交付吞吐、每次新建的平均候选尝试数、后续资源根 SKIP LOCKED 比例、候选复核失效占尝试比例和最终冲突率：

```promql
histogram_quantile(0.95, sum by (le, allocation_type) (rate(remail_allocation_duration_seconds_bucket{result="succeeded"}[5m])))
sum by (allocation_type) (rate(remail_allocation_results_total{result="succeeded"}[1m]))
sum by (allocation_type) (rate(remail_allocation_results_total{result="existing"}[1m]))
sum by (allocation_type) (rate(remail_allocation_results_total{result=~"succeeded|existing"}[1m]))
sum by (allocation_type) (rate(remail_allocation_candidate_attempts_total[5m])) / sum by (allocation_type) (rate(remail_allocation_results_total{result="succeeded"}[5m]))
sum by (allocation_type) (rate(remail_allocation_resource_lock_skips_total[5m])) / sum by (allocation_type) (rate(remail_allocation_candidate_attempts_total[5m]))
sum by (allocation_type) (rate(remail_allocation_candidate_recheck_misses_total[5m])) / sum by (allocation_type) (rate(remail_allocation_candidate_attempts_total[5m]))
sum by (allocation_type) (rate(remail_allocation_results_total{result="conflict"}[5m])) / sum by (allocation_type) (rate(remail_allocation_results_total[5m]))
```
...
`resource_lock_skips_total` 只统计同一事务已经持有资源根后，后续资源根使用 `SKIP LOCKED` 未取得的次数；首个资源根的等待型锁不计入，应结合 Allocation P95 和数据库锁等待观察。
