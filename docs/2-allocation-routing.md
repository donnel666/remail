...
`RoutingCandidate` 是 Microsoft/Domain 候选读模型和后台诊断能力，不是库存扣减表。分配可以使用候选读模型缩小范围，但必须在同一数据库事务内按资源主键重新校验 Core 源表中的资源状态、出售标记或用途、owner 启用状态和 owner 角色，避免 stale candidate 造成错误分配。Domain 候选表保存 `purpose=sale/not_sale + status=normal + mailServer=online` 的资源快照；`sale` 仍需 owner eligible，`not_sale` 只能作为 owner 私有候选。
