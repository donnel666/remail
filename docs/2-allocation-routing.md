...
| 2026-09-11 | V1.14 | Codex | Microsoft 取消全局候选兜底，配置桶未命中后合并查询 100 个不同桶；预算跨事务重试共享，有界未命中立即失败，不重复扫描同一批桶，不作为全局售罄依据。 |
...
| 规则 | 说明 |
|------|------|
| 分桶 | Microsoft 资源使用 2048 桶，每个供给范围和邮箱形态先探测配置数量的桶（默认 4 个）；未命中时将额外 100 个不同桶合并为一次 `IN` 查询，环形回绕，找到即可停止。主邮箱和显式别名各执行一条 SQL，默认每条分配路径最多 10 条候选 SQL。桶预算跨外层重试及仓储事务重试共享，已经查询过的桶不再重查。仓储拒绝空桶列表或超过 100 桶的单批查询。取消 Microsoft 全局候选兜底，有界未命中直接返回库存不足，争用耗尽则返回分配冲突，两者都不证明整个后缀已售罄。域名资源根使用 512 桶，仅承担生成兜底；已生成域名邮箱按规范化邮箱地址的稳定 CRC32 哈希使用独立的 2048 桶。域名分配先复用已生成邮箱，探测不到时再从符合公共后缀的域名资源生成新邮箱。 |
...
项目级库存使用独立的 `alloc:inventory:v6:*` 数据、锁和 active ZSET；v6 商品库存快照包含内部商品类型，不读取缺少该字段的 v5 payload。滚动发布期间不得删除旧 v5 active ZSET；全部旧实例退出且最长十分钟旧刷新任务 drain 后，可执行 `UNLINK alloc:inventory:v5:active`。旧 v5 payload 依靠二十四小时 TTL、旧 marker/lock 依靠十分钟 TTL 自然淘汰。
