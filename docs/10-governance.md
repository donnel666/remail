...
| 2026-07-20 | V1.3 | Codex | 落地管理员系统事件/操作审计的真实分页查询、筛选、操作者展示和按日志流手动清理；普通管理员只读，清理仅允许 super_admin，操作审计清理后保留本次清理记录。 |
...
管理员日志页直接读取 Governance 拥有的两类安全日志，不读取进程原始输出。系统事件按时间、级别、模块、事件、业务对象、摘要和 requestId 展示；操作审计额外以当前用户邮箱补充 operator 展示，并保留不可变的 operatorUserId。两类列表均按 `createdAt,id` 倒序分页，支持时间范围、流内状态和安全字段全文搜索，响应中的 `facets.system/operation` 用于显示两类日志当前总量。

手动清理必须保留：`super_admin` 可按截止时间独立清理 system 或 operation 流，允许选择未来截止时间。删除与新的 OperationLog 必须在同一事务内完成，任何一个接口都不得顺带删除另一类日志；审计写入失败或请求取消时整次清理回滚。清理 operation 流时，本次清理记录在旧记录删除后写入，确保该高风险动作仍可追溯。
...
| `DELETE` | `/v1/admin/logs/system?before={RFC3339}` | 仅 super_admin 按截止时间清理系统日志。 |
| `DELETE` | `/v1/admin/logs/operations?before={RFC3339}` | 仅 super_admin 按截止时间清理操作日志，并在清理后保留本次审计。 |
...
日志读取要求 `governance:log/read`，默认授予 admin 和 super_admin；清理同时要求 `governance:log/operate` 与 `super_admin` 角色，默认只授予 super_admin。查询参数为 `level/result/search/from/to/offset/limit`，其中 `limit` 最大 100、`search` 最大 200 字符。无效枚举、时间或范围返回 400；查询服务不可用返回安全 503，不返回数据库或上游错误原文。
