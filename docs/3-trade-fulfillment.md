...
| 2026-07-09 | V1.1 | Codex | 补充分配优先于扣款的私有库存语义：实际分配到 `owned` 库存时订单应付为 `0.00`，仍必须创建 0 积分消费流水。 |
...
安全边界：所有邮箱类型只交付所选项目的系统收件权限。购买为长效收件，接码为短效且只收一次验证码；用户只能通过系统收件功能取件。资源密码、2FA、应用密码、上游令牌和会话对外只写不读，单笔/批量下单、幂等重试和订单查询均不得返回；`serviceToken` 仅是订单项目的系统收件凭证。2026-09-09 修复了 Gmail 购买交付资源凭据的 P0 错误，任何邮箱类型都不得再引入该例外。
...
| INV-T4 | 进入 `paid` 必须绑定扣款流水；公开库存流水积分为商品价格的负数，私有库存流水积分为 `0.00`，不得因 0 积分跳过流水。 |
