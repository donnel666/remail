...
| INV-AS8 | 提现工单只记录请求，不预冻结或修改钱包；管理员处理时必须重新核对供应商可用余额。 |
...
供应商提现和转入用户钱包同样复用 `POST /v1/tickets`：`ticketType=general`、标题固定为“供应商提现申请”，首条消息记录提现金额、提现去向和用户备注。提现到支付宝时首条消息必须带一张支付宝收款码图片附件；转入用户钱包不要求附件。AFTERSALE 不执行资金操作，管理员核对申请人实时余额后通过 Billing 后台钱包能力人工处理，并在工单中回复和关闭。
