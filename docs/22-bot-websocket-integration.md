...
AstrBot 在 WebSocket 模式下始终由主服务连接发起订阅，群通知目标列表只控制转发。
没有目标时仍消费事件并记录“已接收，未配置发送目标”，不保存目标投递游标；
之后配置新目标时仍从服务端确认的当前游标开始，不自动补发未配置目标期间的事件。
QQ群通知目标支持纯群号：发送时从唯一的 `aiocqhttp` 平台实例读取 ID 并补齐会话前缀。
多 QQ 实例和 Telegram 可用完整会话标识指定平台。游标沿用原配置项，升级后保留失败重试进度。
