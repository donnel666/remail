...
func TestRetentionRepoRedactsOnlyExpiredTerminalGmailCodes(t *testing.T) {
	...
	require.NoError(t, db.Exec(`CREATE TABLE smsbower_orders (
		id INTEGER PRIMARY KEY,
		status TEXT NOT NULL,
		completed_at DATETIME,
		codes_json JSON NOT NULL
	...
	for _, row := range []struct {
		...
		{id: 1, status: "cancelled", completedAt: now.AddDate(0, 0, -4)},
		{id: 2, status: "completed", completedAt: now.AddDate(0, 0, -2)},
		{id: 3, status: "active", completedAt: now.AddDate(0, 0, -4)},
	} {
		require.NoError(t, db.Exec(
			"INSERT INTO smsbower_orders(id, status, completed_at, codes_json) VALUES (?, ?, ?, ?)",
			row.id, row.status, row.completedAt, `[{"seq":1,"code":"654321"}]`,
		).Error)
	}
	...
	values = nil
	require.NoError(t, db.Table("smsbower_orders").Order("id").Pluck("codes_json", &values).Error)
	require.Equal(t, []string{"[]", `[{"seq":1,"code":"654321"}]`, `[{"seq":1,"code":"654321"}]`}, values)
}
