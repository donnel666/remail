...
func TestInventoryCacheServesRedisAndRefreshesActiveEntries(t *testing.T) {
	...
	require.Equal(t, 2, repo.accessCalls, "project visibility must be checked even on a cache hit")
	...
	require.Equal(t, 5*allocapp.InventoryRefreshInterval, server.TTL(inventoryCacheKey(allocapp.InventoryCacheStats, 10, 7)))
	require.Equal(t, 5*allocapp.InventoryRefreshInterval, server.TTL(inventoryCacheKey(allocapp.InventoryCacheProducts, 10, 7)))
...
func TestInventoryCacheCollapsesConcurrentColdMisses(t *testing.T) {
	server := miniredis.RunT(t)
	client := redis.NewClient(&redis.Options{Addr: server.Addr()})
	t.Cleanup(func() { require.NoError(t, client.Close()) })
...
func TestInventoryRefreshBatchIsBounded(t *testing.T) {
	...
	result, err := useCase.RefreshInventoryCache(context.Background())
	require.NoError(t, err)
	require.Equal(t, 100, result.Attempted)
