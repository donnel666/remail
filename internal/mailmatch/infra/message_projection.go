...
func (r *Repo) ListUnprojectedMessages(ctx context.Context, resourceType domain.ResourceType, emailResourceIDs []uint, limit int) ([]domain.Message, error) {
	if (resourceType != domain.ResourceTypeMicrosoft && resourceType != domain.ResourceTypeGmail && resourceType != domain.ResourceTypeICloud && resourceType != domain.ResourceTypeProto) || len(emailResourceIDs) == 0 || limit <= 0 {
		return nil, nil
	}
	var rows []MessageModel
	if err := r.dbFor(ctx).
