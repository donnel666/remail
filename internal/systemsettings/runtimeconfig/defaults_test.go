...
func TestDefaultSettingsAreValidAndIndependent(t *testing.T) {
	...
	require.Equal(t, "100", keys[DomainSaleQualityMinOrdersKey])
	require.Equal(t, "60", keys[DomainSaleQualityMinSuccessPercentKey])
	require.Equal(t, "1", keys[DomainSaleQualityWindowHoursKey])
	require.Equal(t, "30", keys[DomainSaleQualityCheckIntervalSecondsKey])
	...
	require.Equal(t, "24", keys[ICloudPhoneBlacklistHoursKey])
	...
	require.Equal(t, "8", keys["default_project_gmail_variant_code_price"])
	require.Equal(t, "true", keys["default_project_gmail_variant_code_enabled"])
	require.Equal(t, "false", keys["default_project_gmail_variant_purchase_enabled"])
