...
func TestValidateEmailServiceSettings(t *testing.T) {
	...
	require.NoError(t, Validate(GmailVariantCooldownMinutesKey, "0"))
	require.NoError(t, Validate(GmailVariantCooldownMinutesKey, "1440"))
	require.ErrorIs(t, Validate(GmailVariantCooldownMinutesKey, "1441"), domain.ErrInvalidValue)
...
func TestValidateRechargePaymentSettings(t *testing.T) {
	...
	require.NoError(t, Validate("customer_service_qq_group_number", ""))
	require.ErrorIs(t, Validate("customer_service_qq_group_number", "1234a"), domain.ErrInvalidValue)
	require.NoError(t, Validate("customer_service_qq_group_url", "https://qm.qq.com/q/example"))
	require.NoError(t, Validate("customer_service_telegram_group_url", "https://t.me/example"))
	require.ErrorIs(t, Validate("customer_service_telegram_group_url", "javascript:alert(1)"), domain.ErrInvalidValue)
	...
	require.NoError(t, Validate("epusdt_minimum_payment_amount", "12.50"))
	require.NoError(t, Validate("epusdt_minimum_payment_amount", "0.02"))
	require.ErrorIs(t, Validate("epusdt_minimum_payment_amount", "0"), domain.ErrInvalidValue)
	require.ErrorIs(t, Validate("epusdt_minimum_payment_amount", "0.01"), domain.ErrInvalidValue)
	require.ErrorIs(t, Validate("epusdt_minimum_payment_amount", "12.501"), domain.ErrInvalidValue)
...
func TestValidateSystemOperationsSettings(t *testing.T) {
	...
	for _, key := range []string{
		...
		"asynq_queue_background_validation_weight", "asynq_queue_background_gmail_validation_weight", "asynq_queue_background_icloud_validation_weight", "asynq_queue_background_domain_validation_weight", "asynq_queue_background_alias_weight",
