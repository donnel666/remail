...
func NewViewRepo(db *gorm.DB) *ViewRepo { return &ViewRepo{db: db} }
...
func (r *ViewRepo) OrderBuckets(ctx context.Context, userID uint, sqlFormat string, from, to time.Time) ([]dashboardapp.OrderBucketRow, error) {
	...
	sel := fmt.Sprintf(
		"DATE_FORMAT(created_at, '%s') AS bucket, COUNT(*) AS orders, COALESCE(SUM(service_mode = 'code'),0) AS code_orders, COALESCE(SUM(pay_amount - refund_amount),0) AS spend",
		sqlFormat,
	)
	var rows []struct {
...
// Leaderboard ranks users by code-receipt count (code-mode orders only, so
// purchase-order mail deliveries are excluded). since=nil is all-time.
// ponytail: live GROUP BY over the delivery heads; fine at current volume, add a
// rollup table if the all-time scan gets slow.
...
// UserStanding returns the caller's own code-receipt count and their ordinal
...
func (r *ViewRepo) UserStanding(ctx context.Context, userID uint, since *time.Time) (dashboardapp.Standing, error) {
	countQuery := r.db.WithContext(ctx).
		Table("mailmatch_order_delivery_heads AS h").
		Joins("JOIN orders AS o ON o.id = h.order_id").
		Where("o.user_id = ? AND o.service_mode = 'code'", userID)
	if since != nil {
