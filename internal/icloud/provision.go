...
func (s *Service) ProcessICloudProvision(ctx context.Context, task iCloudProvisionTask) (resultErr error) {
	...
	for _, kind := range []string{iCloudChannelAppleAccount, iCloudChannelWeb} {
		...
		*channel = updated
		...
		if resourceNeedsCookies && updated.NextKeepaliveAt != nil {
...
func (s *Service) finishICloudProvision(ctx context.Context, resource iCloudResourceModel, nextAt, now time.Time) (bool, error) {
	...
	err := s.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		...
		refreshCreated, err = s.ensureICloudCookieMaintenanceTx(ctx, tx, locked.ID, false, false)
		if err != nil {
			return err
		}
		...
		if locked.AliasCount >= iCloudMaxAliases {
			if err := clearICloudMaintenanceAtAliasLimitTx(ctx, tx, locked.ID, now); err != nil {
				return err
			}
		}
		if !nextAt.IsZero() {
