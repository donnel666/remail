...
func (f *appleOnboardingFlow) prepareICloud(request AppleOnboardingRequest) (_ AppleOnboardingResponse, resultErr error) {
	...
	case status == http.StatusConflict && authType == "hsa2":
		if err := f.prepareVerification(request); err != nil {
			return AppleOnboardingResponse{}, err
		}
...
func (f *appleOnboardingFlow) sendSMS(request AppleOnboardingRequest) (AppleOnboardingResponse, error) {
	if request.UseDeviceCode {
		if request.SMSPurpose == appleSMSPhoneEnrollment {
			return AppleOnboardingResponse{}, errors.New("phone enrollment requires its initial SMS verification")
		}
		// Trusted-device codes are generated by the bound device. Do not call
		// Apple's SMS dispatch endpoint when device authentication is selected.
...
func (f *appleOnboardingFlow) verifySMS(request AppleOnboardingRequest) (AppleOnboardingResponse, error) {
	if request.UseDeviceCode {
		return f.verifyDeviceCode(request)
	}
...
func (f *appleOnboardingFlow) verifyDeviceCode(request AppleOnboardingRequest) (AppleOnboardingResponse, error) {
	if request.SMSPurpose == appleSMSPhoneEnrollment {
		return AppleOnboardingResponse{}, errors.New("phone enrollment requires its initial SMS verification")
	}
	code := strings.TrimSpace(request.Code)
	if len(code) != 6 || strings.Trim(code, "0123456789") != "" {
		return AppleOnboardingResponse{}, &AppleOnboardingError{Category: "verification_code_missing", SafeMessage: "A six-digit device code is required.", CodeRejected: true}
	}
	body, err := f.request(http.MethodPost, strings.TrimRight(f.state.ServiceURL, "/")+"/auth/verify/trusteddevice/securitycode",
		map[string]any{"securityCode": map[string]any{"code": code}}, false, false, false, true, false, "application/json, text/javascript, */*; q=0.01")
	if err != nil {
		return AppleOnboardingResponse{}, err
	}
	if f.state.Status == http.StatusUnauthorized || f.state.Status == http.StatusForbidden {
	...
	if f.state.Status != http.StatusOK && f.state.Status != http.StatusNoContent {
		return AppleOnboardingResponse{}, &AppleOnboardingError{Category: "verification_code_rejected", SafeMessage: "Apple rejected the device verification code.", CodeRejected: true}
	}
	if len(strings.TrimSpace(string(body))) > 0 {
		data, err := decodeAppleOnboardingObject(body, "verify/trusteddevice/securitycode")
		if err != nil {
			return AppleOnboardingResponse{}, err
		}
		if valid, present := appleOnboardingOptionalBool(appleOnboardingMap(data["securityCode"])["valid"]); present && !valid {
			return AppleOnboardingResponse{}, &AppleOnboardingError{Category: "verification_code_rejected", SafeMessage: "Apple rejected the device verification code.", CodeRejected: true}
		}
	}
	if err := f.finishHSA2Login(request.SMSPurpose); err != nil {
