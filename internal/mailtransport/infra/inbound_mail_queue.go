...
func (q *InboundMailQueue) EnqueueInboundDispatch(ctx context.Context, delay time.Duration) error {
	...
	options := []asynq.Option{
		...
		asynq.TaskID(TypeInboundDispatch),
	}
	if delay > 0 {
...
func (q *InboundMailQueue) EnqueueInboundProcess(ctx context.Context, task mailapp.InboundProcessTask) error {
	if q == nil || q.client == nil {
		return fmt.Errorf("inbound mail queue is unavailable")
	}
	payload, err := json.Marshal(task)
	if err != nil {
		return fmt.Errorf("marshal inbound mail task: %w", err)
	}
	...
	_, err = q.client.EnqueueContext(
		...
		asynqTask,
		asynq.Queue(mailQueueName),
		asynq.TaskID(fmt.Sprintf("%s:%d", TypeInboundProcess, task.InboundMailID)),
	...
	if err != nil {
		...
		return fmt.Errorf("enqueue inbound mail task: %w", err)
	}
	return nil
}
